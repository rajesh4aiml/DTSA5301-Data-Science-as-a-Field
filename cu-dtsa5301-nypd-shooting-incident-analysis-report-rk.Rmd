...
tdRemoveCols <- function(ds_nyc_sh) {
  required_cols <- c("OCCUR_DATE", "OCCUR_TIME", "BORO", "PRECINCT", "LOCATION_DESC", "STATISTICAL_MURDER_FLAG", "PERP_AGE_GROUP", "PERP_SEX", "PERP_RACE", "VIC_AGE_GROUP", "VIC_SEX", "VIC_RACE")
...
nyc_incidents_by_loc_pie_chart_vz <- nyc_incidents_by_loc_df  %>%
  ...
  theme(
    ...
    axis.text.x = element_text(size=0.1) # hide lables
  ) +
...
# Get the precent for the precinct for displaying in the label.
nyc_top_n_precinct_pct <- round(100 * (nyc_top_n_precinct_total/total_no_of_incidents_fl),2)

# label for graph
lbl_prec <- str_c("S-Inc Top 5 precincts by each Boro: \nIncidents", formatNumber(nyc_top_n_precinct_total), " of ", formatNumber(total_no_of_incidents), " (", nyc_top_n_precinct_pct, "%)")

# Prepare he graph for Top 5 precinct
...
nyc_top_n_precinct_ts_vz <- nyc_top_n_precinct_ts_df %>%
  ...
  labs(
    title = str_c("S-Inc Top 5 precincts by each Boro: \nIncidents", formatNumber(nyc_top_n_precinct_total), " of ", formatNumber(total_no_of_incidents),
...
##### **Section 2B: Summarize the Tidy dataset**: nyc_dataset_df. Rinse and repeat tidy-analyze steps until the data is in good order.
...
```{r vz-nyc-prereq, message=FALSE}
sm_cols <- c("OCCUR_DATE", "BORO", "PRECINCT", "LOCATION_DESC", "STATISTICAL_MURDER_FLAG", "PERP_AGE_GROUP", "PERP_SEX", "PERP_RACE", "VIC_AGE_GROUP", "VIC_SEX", "VIC_RACE")
...
nyc_incidents_by_loc_pie_chart_vz
...
```{r vz-nyc-inc-precinct-2, message=FALSE, echo=FALSE}
nyc_top_n_precinct_ts_vz
```
...
```{r vz-nyc-inc-precinct-3, message=FALSE, echo=FALSE}
...
To overcome implicit biases, we need to follow a disciplined and mindful approach of thoroughly analyzing the data, drawing inferences from data, asking questions, collecting more data and validating our assumptions using some quantitative models. 
...
- We reported our observations at each step. Our key observations were:
  ...
  * The top 10 precincts in Brooklyn resulted in a crime rate of **`r bk_top10_global_pct`** of the entire NYC shootings. These precincts: **`r top10_precinct`** had total **`r formatNumber(bk_top10_share)`** shooting incidents and hence they really need attention. 
...
- The crime rate in NYC can be reduced by **more than half** if NYPD and Law enforcement agencies can partner with other Government and private agencies to improve the quality of life in the **Top five precincts in each BORO** shared in [section 3F](#section-3F).
